...
def solve_amount_lin_task():
    def reset():
        ...
        amount_width_entry.config(state="readonly")
        amount_width_entry.delete(0, 'end')
    ...
    def solve_problem():
        ...
        if count < 2:
            ...
            max_count = int(amount_width_entry.get())
            items = []
            for i in range(count):
                items.append([len_details[i], prise_details[i], i + 1])
            items.sort(key=lambda z: z[1] / z[0], reverse=True)
            cur_list.append([0, 0, 0])
            i = 1
            while i * items[0][0] <= stick_len and i <= max_count:
                cur_list.append([i * items[0][0], i*items[0][1], items[0][2]])
                i = i + 1
            i = 1
            while i < count:
                for el in cur_list:
                    new_list.append([el[0], el[1], el[2]])
                    j = 1
                    while el[0] + j * items[i][0] <= stick_len and j <= max_count:
                        new_list.append([el[0] + j * items[i][0], el[1] + j * items[i][1], items[i][2]])
                        j = j + 1
    ...
    def add_detail():
        if stick_length_entry.get() == '' or amount_width_entry.get() == '':
            messagebox.showerror("Ошибка", "Прежде чем заполнять детали введите длину стержня и максимальное кол-во.")
            return
    ...
    label_amount = tk.Label(win2, text="Введите максимальное \nкол-во деталей 1 типа: ", font=13)
    label_amount.place(x=10, y=80)
